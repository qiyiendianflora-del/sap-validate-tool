...
# -*- coding: utf-8 -*-
"""
SAP凭证校验工具 v2.2 - PyQt5 GUI（橙色主题 + 表格校验）
...
# ━━━━━━━ 常量 ━━━━━━━
APP_NAME="SAP凭证校验工具"; VERSION="v2.2"; DATA_ROW=4
...
O_IDX=column_index_from_string('O')-1; AK_IDX=column_index_from_string('AK')-1
...
def run_validate_data(all_rows, rule_map, recon_map, mapping_data=None, progress_cb=None):
    """核心校验：接收 {row_num: [values...]} 格式的数据"""
    mapping_sets={}
    if mapping_data:
        for a,combos in mapping_data.items():
            mapping_sets[a]=set(tuple(c) for c in combos)
...
class DropArea(QLabel):
    ...
    def __init__(self,text="拖拽文件到此处"):
        super().__init__(text);self.setAcceptDrops(True);self.setAlignment(Qt.AlignCenter);self.setFixedHeight(46);self._idle()
    def _idle(self): self.setStyleSheet(f"QLabel{{border:2px dashed {C_ACCENT};border-radius:8px;color:{C_TEXT_SEC};background:{C_PRIMARY_XL};font-size:12px;}}")
    def _loaded(self,fn):
        self.setText(f"📄 {fn}");self.setStyleSheet(f"QLabel{{border:2px solid {C_SUCCESS};border-radius:8px;color:{C_SUCCESS};background:#E8F5E9;font-size:12px;font-weight:bold;}}")
    ...
    def dropEvent(self,e):
        ...
        if urls:
            ...
            if p.lower().endswith(('.xlsx','.xls')): self._loaded(os.path.basename(p));self.file_dropped.emit(p)
            else: QMessageBox.warning(self,"格式错误","请拖入Excel文件");self._idle()
...
class MainWindow(QMainWindow):
    ...
    def _load_saved(self):
        ...
        if rp and os.path.exists(rp): self.rule_file=rp;self.r_drop._loaded(os.path.basename(rp))
        mp=cfg.get("mapping_file","")
        if mp and os.path.exists(mp): self.mapping_file=mp;self.m_drop._loaded(os.path.basename(mp))
        cached=load_mapping_cache()
        if cached:
            self.mapping_data=cached;cnt=sum(len(v) for v in cached.values())
            self.m_info.setText(f"✅ 已加载 {len(cached)} 个科目 {cnt} 条规则")
            self.m_info.setStyleSheet(f"color:{C_SUCCESS};font-size:10px;padding:2px 0;")
        bp=cfg.get("b_file","")
        if bp and os.path.exists(bp): self.b_file=bp;self.b_drop._loaded(os.path.basename(bp))
    ...
    def _init_ui(self):
        self.setWindowTitle(f"{APP_NAME}  {VERSION}");self.setMinimumSize(1060,760);self.resize(1200,860)
        self.setStyleSheet(f"""
            QMainWindow{{background:{C_BG};}}
            QGroupBox{{font-weight:bold;font-size:12px;color:{C_TEXT};border:1px solid {C_BORDER};border-radius:8px;margin-top:12px;padding-top:18px;background:{C_CARD};}}
        ...
        root=QVBoxLayout(central);root.setSpacing(8);root.setContentsMargins(16,10,16,10)
        ...
        self.page_tab.setStyleSheet(f"""
            QTabBar::tab{{background:{C_CARD};border:1px solid {C_BORDER};border-bottom:none;
                border-radius:8px 8px 0 0;padding:8px 24px;margin-right:4px;font-size:13px;font-weight:bold;}}
        ...
        self.page_tab.currentChanged.connect(self._page_changed)
    ...
    def _page_changed(self,idx): self.stack.setCurrentIndex(idx)

    # ── 页面一：文件校验 ──
    def _build_file_page(self):
        page=QWidget();root=QVBoxLayout(page);root.setSpacing(8);root.setContentsMargins(0,4,0,0)
        # 上传区
        ...
        for lbl_text,attr,slot,drop_text in [
            ...
            row=QHBoxLayout();l=QLabel(lbl_text);l.setFixedWidth(90)
            da=DropArea(drop_text);setattr(self,attr,da)
            b=QPushButton("选择");b.setFixedWidth(60);b.setStyleSheet(self._bs(C_PRIMARY))
            ...
            row.addWidget(l);row.addWidget(da,1);row.addWidget(b);ul.addLayout(row)
        self.m_info=QLabel("");self.m_info.setStyleSheet(f"color:{C_TEXT_SEC};font-size:10px;");ul.addWidget(self.m_info)
        root.addWidget(ub)
        # 按钮
        brow=QHBoxLayout()
        self.vbtn=QPushButton("  🔍  开始校验");self.vbtn.setFixedHeight(38);self.vbtn.setStyleSheet(self._bs(C_SUCCESS,12,24));self.vbtn.clicked.connect(self._start)
        self.dbtn=QPushButton("  ⬇️  下载报告");self.dbtn.setFixedHeight(38);self.dbtn.setStyleSheet(self._bs(C_PRIMARY,12,24));self.dbtn.setEnabled(False);self.dbtn.clicked.connect(self._download)
        brow.addWidget(self.vbtn);brow.addSpacing(8);brow.addWidget(self.dbtn);brow.addStretch();root.addLayout(brow)
        self.prog=QProgressBar();self.prog.setFixedHeight(6);self.prog.setValue(0);self.prog.setTextVisible(False)
        self.prog.setStyleSheet(f"QProgressBar{{background:#e0e0e0;border-radius:3px;}}QProgressBar::chunk{{background:{C_ACCENT};border-radius:3px;}}");root.addWidget(self.prog)
        sr=QHBoxLayout();sr.setSpacing(10)
        self.ct=StatCard("📊","总行数","—",C_TEXT);self.co=StatCard("✅","通过","—",C_SUCCESS)
        self.ce=StatCard("❌","错误","—",C_ERROR);self.cw=StatCard("⚠️","警告","—",C_WARN)
        for c in [self.ct,self.co,self.ce,self.cw]: sr.addWidget(c)
        root.addLayout(sr)
        rb2=QGroupBox("  校验结果");rl2=QVBoxLayout(rb2)
        self.tabs=QTabBar()
        self.tabs.setStyleSheet(f"QTabBar::tab{{background:{C_CARD};border:1px solid {C_BORDER};border-bottom:none;border-radius:6px 6px 0 0;padding:5px 14px;margin-right:2px;font-size:11px;}}QTabBar::tab:selected{{background:{C_PRIMARY};color:white;font-weight:bold;}}QTabBar::tab:hover{{background:{C_PRIMARY_XL};}}")
        for t in ["全部","必输项","借贷平衡","记账码","禁填字段","费用类别","⚠️ 警告"]: self.tabs.addTab(t)
        self.tabs.currentChanged.connect(self._tab_changed);rl2.addWidget(self.tabs)
        self.tbl=QTableWidget();self.tbl.setColumnCount(4);self.tbl.setHorizontalHeaderLabels(["行号","总账科目","错误类型","错误详情"])
        self.tbl.horizontalHeader().setSectionResizeMode(3,QHeaderView.Stretch);self.tbl.setColumnWidth(0,60);self.tbl.setColumnWidth(1,120);self.tbl.setColumnWidth(2,140)
        self.tbl.setEditTriggers(QAbstractItemView.NoEditTriggers);self.tbl.setAlternatingRowColors(True);self.tbl.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.tbl.setStyleSheet(f"QTableWidget{{border:1px solid {C_BORDER};gridline-color:#eee;}}QHeaderView::section{{background:{C_PRIMARY};color:white;font-weight:bold;padding:6px;border:none;}}QTableWidget::item:alternate{{background:#FFF8F0;}}")
        rl2.addWidget(self.tbl);root.addWidget(rb2,1)
        return page
    ...
    def _build_grid_page(self):
        ...
        # 按钮栏
        brow=QHBoxLayout()
        self.g_vbtn=QPushButton("  🔍  校验");self.g_vbtn.setFixedHeight(36);self.g_vbtn.setStyleSheet(self._bs(C_SUCCESS,11,20));self.g_vbtn.clicked.connect(self._grid_validate)
        self.g_clr=QPushButton("  🗑️  清空数据");self.g_clr.setFixedHeight(36);self.g_clr.setStyleSheet(self._bs("#757575",11,20));self.g_clr.clicked.connect(self._grid_clear)
        self.g_dl=QPushButton("  ⬇️  下载报告");self.g_dl.setFixedHeight(36);self.g_dl.setStyleSheet(self._bs(C_PRIMARY,11,20));self.g_dl.setEnabled(False);self.g_dl.clicked.connect(self._grid_download)
        self.g_info=QLabel("从 Excel / SAP 复制数据后，选中第4行第1个单元格，Ctrl+V 粘贴")
        self.g_info.setStyleSheet(f"color:{C_TEXT_SEC};font-size:11px;")
        brow.addWidget(self.g_vbtn);brow.addSpacing(6);brow.addWidget(self.g_clr);brow.addSpacing(6);brow.addWidget(self.g_dl);brow.addSpacing(12);brow.addWidget(self.g_info);brow.addStretch()
        root.addLayout(brow)
        # 进度条
        self.g_prog=QProgressBar();self.g_prog.setFixedHeight(6);self.g_prog.setValue(0);self.g_prog.setTextVisible(False)
        self.g_prog.setStyleSheet(f"QProgressBar{{background:#e0e0e0;border-radius:3px;}}QProgressBar::chunk{{background:{C_ACCENT};border-radius:3px;}}");root.addWidget(self.g_prog)
        # 统计
        sr=QHBoxLayout();sr.setSpacing(10)
        self.g_ct=StatCard("📊","总行数","—",C_TEXT);self.g_co=StatCard("✅","通过","—",C_SUCCESS)
        self.g_ce=StatCard("❌","错误","—",C_ERROR);self.g_cw=StatCard("⚠️","警告","—",C_WARN)
        for c in [self.g_ct,self.g_co,self.g_ce,self.g_cw]: sr.addWidget(c)
        root.addLayout(sr)
        # 大表格
        nc=NUM_COLS+1  # 多一列错误说明
        self.grid=QTableWidget();self.grid.setColumnCount(nc)
        hlabels=COL_LETTERS+["错误说明"]
        self.grid.setHorizontalHeaderLabels(hlabels)
        # 前3行表头 + 200行空数据
        ...
        for r,hrow in enumerate([HEADER_ROW1,HEADER_ROW2,HEADER_ROW3]):
            for c,v in enumerate(hrow):
                ...
                item.setBackground(QColor("#E0E0E0") if r==0 else QColor("#F5F5F5"))
                item.setFont(QFont("微软雅黑",9,QFont.Bold if r==0 else QFont.Normal))
        ...
        self.grid.setStyleSheet(f"QTableWidget{{border:1px solid {C_BORDER};gridline-color:#ddd;font-size:10px;}}QHeaderView::section{{background:{C_PRIMARY};color:white;font-weight:bold;padding:4px;border:none;font-size:10px;}}")
        self.grid.horizontalHeader().setDefaultSectionSize(80)
        self.grid.setColumnWidth(NUM_COLS,250)  # 错误说明列宽
        self.grid.verticalHeader().setDefaultSectionSize(22)
        self.grid.setAlternatingRowColors(True)
        # 冻结前3行效果：滚动时表头行始终可见通过 freezeTableWidget 或简单行高设置
        root.addWidget(self.grid,1)
        # 悬停提示
        self.grid.cellEntered.connect(self._grid_cell_hover)
        self.grid.setMouseTracking(True)
        self.grid_errors={}  # {(row,col_letter): error_msg}
    ...
    def _grid_cell_hover(self,row,col):
        if col<NUM_COLS:
            ...
            if key in self.grid_errors:
                QToolTip.showText(self.grid.mapToGlobal(QPoint(0,0)),self.grid_errors[key])
    ...
    def _grid_clear(self):
        for r in range(3,self.grid.rowCount()):
            for c in range(self.grid.columnCount()):
                ...
                if item:
                    item.setText("");item.setBackground(QColor("white"));item.setToolTip("")
                else:
        ...
        self.g_info.setText("已清空。粘贴新数据后点击「校验」")
    ...
    def _grid_done(self,result):
        ...
        ec=len(errors);wc=len([w for w in warnings if w[2]=='科目不在规则表']);ok=max(0,total-ec-wc)
        self.g_ct.set_value(total);self.g_co.set_value(ok);self.g_ce.set_value(ec);self.g_cw.set_value(wc)
        ...
        for r in range(3,self.grid.rowCount()):
            for c in range(self.grid.columnCount()):
                item=self.grid.item(r,c)
                if item:
                    item.setBackground(QColor("white"));item.setToolTip("")
        ...
        # 标红 + 写错误说明
        for r,el in errors.items():
            grid_row=r-1  # excel行号→grid行号（grid从0开始，excel从1开始）
            if grid_row>=self.grid.rowCount(): continue
            # 整行标黄
            for c in range(NUM_COLS):
                item=self.grid.item(grid_row,c)
                if item:
                    cl=COL_LETTERS[c]
                    if (r,cl) in ecells:
                        ...
                        if msgs:
                            tip="\n".join(msgs)
                            item.setToolTip(tip)
                            self.grid_errors[(grid_row,cl)]=tip
                    else:
                        item.setBackground(QColor("#FFF2CC"))
            # 错误说明列
            msgs=[e[2] for e in el]
            err_item=self.grid.item(grid_row,NUM_COLS)
            if not err_item:
                err_item=QTableWidgetItem()
                self.grid.setItem(grid_row,NUM_COLS,err_item)
            err_item.setText(" | ".join(msgs))
            err_item.setForeground(QColor(C_ERROR))
            err_item.setBackground(QColor("#FFF2CC"))
        ...
        for r,acct in wa.items():
            ...
            err_item=self.grid.item(grid_row,NUM_COLS)
            if not err_item:
                err_item=QTableWidgetItem();self.grid.setItem(grid_row,NUM_COLS,err_item)
            err_item.setText(f"⚠️ 科目{acct}不在规则表中")
            err_item.setForeground(QColor(C_WARN));err_item.setBackground(QColor("#FFE0B2"))

        self.grid_result=result;self.g_vbtn.setEnabled(True);self.g_dl.setEnabled(True)
        self.g_info.setText(f"校验完成：{ec}个错误 {wc}个警告  {datetime.now().strftime('%H:%M:%S')}")
    ...
    def _grid_download(self):
        ...
        errors=self.grid_result["errors"];warnings=self.grid_result["warnings"]
        rm,_=load_rule_table(self.rule_file)
        total=self.grid_result["total"]
        out_wb=build_report(errors,warnings,wb,ws,rm,total)
        dn=f"校验报告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        sp,_=QFileDialog.getSaveFileName(self,"保存",dn,"Excel (*.xlsx)")
        if sp:
            try: out_wb.save(sp);QMessageBox.information(self,"✅","报告已保存到：\n"+sp)
            except Exception as e: QMessageBox.critical(self,"失败",str(e))

    # ── 公共方法 ──
    def _bs(self,color,sz=10,px=16):
        return f"QPushButton{{background:{color};color:white;border-radius:6px;padding:6px {px}px;font-size:{sz}px;font-weight:bold;border:none;}}QPushButton:hover{{background:{color}dd;}}QPushButton:pressed{{background:{color}bb;}}QPushButton:disabled{{background:#ccc;color:#888;}}"
    ...
    def _load_mapping(self,path):
        try:
            ...
            save_mapping_cache(data);self._save_paths();self.m_drop._loaded(os.path.basename(path))
            cnt=sum(len(v) for v in data.values())
            self.m_info.setText(f"✅ 已加载并缓存：{len(data)} 个科目 {cnt} 条规则")
            self.m_info.setStyleSheet(f"color:{C_SUCCESS};font-size:10px;padding:2px 0;")
        except Exception as ex: QMessageBox.critical(self,"加载失败",str(ex))
    ...
    def _start(self):
        ...
        self.vbtn.setEnabled(False);self.dbtn.setEnabled(False);self.prog.setValue(0);self.tbl.setRowCount(0)
    ...
    def _done(self,result):
        self.result=result;errs=result["errors"];warns=result["warnings"];total=result["total"]
        ec=len(errs);wc=len([w for w in warns if w[2]=='科目不在规则表']);ok=max(0,total-ec-wc)
        ...
        for w in warns:
            if w[2]=='科目不在规则表': self.all_table_rows.append((str(w[0]),w[1],"⚠️ 科目不在规则表","请核实",'警告',True))
            elif '记账码' in w[2]: self.all_table_rows.append((str(w[0]),'',f"⚠️ {w[2]}",str(w[1]),'记账码',True))
    ...
    def _download(self):
        ...
        sp,_=QFileDialog.getSaveFileName(self,"保存",dn,"Excel (*.xlsx)")
        if sp:
            try: self.result["out_wb"].save(sp);QMessageBox.information(self,"✅","报告已保存到：\n"+sp)
            except Exception as e: QMessageBox.critical(self,"失败",str(e))
